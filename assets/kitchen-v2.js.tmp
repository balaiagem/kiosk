...
(function () {
  ...
  function render(orders) {
    ...
    // Skip rebuilding the grid (the flicker-causing part) when nothing
    // meaningful has changed since the last poll. We hash the order
    // identities + statuses + line counts; if the hash matches, bail.
    const hash = orders.map(o => {
      return o.id + ':' + o.status + ':' + ((o.lines || []).length);
    }).join('|');
